...
OpenSpending.App.SpendBrowser = (function($) {
  ...
  return function(config) {
    var my = {
	  ...
	  if ((config.rootvar == true) || (typeof config.swg == 'undefined')) {
	  ...
	  // if it's not root, show the particular SWG
		...
		if (config.isasectorobjective == true) {
		    searchtext = " " + config.year + " in " + config.sector_objective;
		    var params = {
	            q: '',
	            qparams: {
	              to: '*' + to + '*',
	              dataset: config.dataset,
	              'subsector.name': config.sector_objective,
	              'time.year': config.year
	            },
	            sort: 'amount desc',
	            facet: true,
		    ...
		    searchtext = " " + config.year + " in " + config.swg;
		    var params = {
	            q: '',
	            qparams: {
	              to: '*' + to + '*',
	              dataset: config.dataset,
	              'sector.name': config.swg,
	              'time.year': config.year
	            },
	            sort: 'amount desc',
	            facet: true,
